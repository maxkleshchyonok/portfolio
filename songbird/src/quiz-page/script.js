// import birdsAllInfo from "../store";
...
let playButton = document.querySelector('.buttons_play');
let nextButton = document.querySelector('.next-button');
let questionImage = document.querySelector('.question_image');
...
function loadTrack(trackIndex) {
    ...
    currentTrack.src = birdsAllInfo[trackIndex].audio;
    // questionImage.src = birdsAllInfo[trackIndex].image
    questionImage.src = '../assets/images/question-sign-1.png';
    currentTrack.load();
    birdName.textContent = birdsAllInfo[trackIndex].name;
...
function nextTrack() {
    if(trackIndex < birdsAllInfo.length - 1 && isRandom === false){
        trackIndex += 1;
    } else if(trackIndex < birdsAllInfo.length - 1 && isRandom === true){
        let random_index = Number.parseInt(Math.random() * birdsAllInfo.length);
...
function hideInfo() {
    ...
    infoImageName.classList.add('hide-image-and-names')
    infoDescription.classList.add('description-hide')
...
function uploadQuiz() {
    for (let i=0;i<birdsAllInfo.length;i++){
        ...
        let birdImage = document.querySelector('.answer-image');
        let birdInfoName = document.querySelector('.answer-bird-name');
        ...
        label.appendChild(liInput);
        nodeLi.appendChild(label);
        document.querySelector('.answers_list').appendChild(nodeLi);

        // birdImage.src = birdsAllInfo[Number(inputid)].image;
        liInput.addEventListener('click', ()=>{
            if(label.textContent === birdName.textContent){
            ...
            birdImage.src = birdsAllInfo[Number(inputid)].image;
            birdInfoName.textContent = birdsAllInfo[Number(inputid)].name
            birdSpecies.textContent = birdsAllInfo[Number(inputid)].species
            birdInfo.textContent = birdsAllInfo[Number(inputid)].description
